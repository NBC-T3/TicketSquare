...
class MainViewController: UIViewController {
    ...
    private func setupViews() {
        ...
        collectionView.register(
            ...
            forCellWithReuseIdentifier: PagingImageCell.identifier)
        collectionView.register(
            SmallImageCell.self,
            forCellWithReuseIdentifier: SmallImageCell.identifier)
    }
...
extension MainViewController: UICollectionViewDataSource {
    ...
    func collectionView(
        ...
        switch section {
        case 0:
            return bigImageURLs.count // 큰 이미지 섹션
        case 1:
            return smallImageURLs.count // 작은 이미지 섹션 1
        case 2:
            return smallImageURLs.count // 작은 이미지 섹션 2
        case 3:
            return smallImageURLs.count // 작은 이미지 섹션 3
        default:
            return 0
        }
    }
    ...
    func collectionView(
        ...
        case 0:  // 큰 이미지 섹션
            ...
            if indexPath.item < bigImageURLs.count {
                let urlString = bigImageURLs[indexPath.item]  // 안전하게 접근
                cell.configure(with: urlString)
            }
            return cell

        case 1, 2, 3: // 작은 이미지 섹션 1, 2, 3
            let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: SmallImageCell.identifier,
                for: indexPath) as! SmallImageCell
            let urlString = smallImageURLs[indexPath.item]

            // 첫 번째 셀에만 제목 표시
            let showTitle = (indexPath.item == 0)
            cell.configure(with: urlString)
            return cell
